...
use std::{cell::Cell, fmt, mem::discriminant, rc::Rc};
...
impl Value {
    ...
    pub fn add(self, other: Value) -> Value {
        match (&self, &other) {
            (Value::Number(s), Value::Number(o)) => Value::Number(s + o),
            _ => Value::DynString(Rc::from(
    ...
    pub fn sub(self, other: Value) -> Option<Value> {
        match (&self, &other) {
            (Value::Number(s), Value::Number(o)) => Some(Value::Number(s - o)),
            _ => None
        }
    }
